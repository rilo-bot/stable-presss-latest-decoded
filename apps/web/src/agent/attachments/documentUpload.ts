...
import type { DocAttachment, DocDigest } from './docTypes';

/**
 * Local id for an attachment. Was imported from the v1 editor's `applyEdits`,
 * which went with that builder — it is three lines and this is the only caller.
 * These ids never leave the browser session, so uniqueness within one composer is
