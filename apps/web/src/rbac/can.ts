...
/** Party record ids whose claim is still awaiting verification (view-only). */
...
/**
 * Horse ids the user may PREVIEW read-only — the authorised set (verified parties
 * + orgs) PLUS horses linked to a party whose claim is still PENDING. Pending
 * scope is view-only: it never feeds canManageHorse, so writes stay gated to the
 * verified `authorisedHorseIds` set. Used to let a not-yet-verified claimant look
 * around the stable they're about to manage.
 */
export function previewHorseIds(
  ...
  pendingPartyIds(user).forEach((pid) =>
    horsesLinkedToParty(pid, data, { currentOnly: true }).forEach((h) => ids.add(h)),
  );
  return Array.from(ids);
}
...
/**
 * Can the user edit this party's profile? Staff, or the account that holds a
 * VERIFIED claim on it (their own party identity — the hub of their self-service).
 */
export function canManageParty(
  ...
  if (isStaff(user)) return true;
  return verifiedPartyIds(user).includes(partyId);
}

/** The user's primary party id (first verified claim), if any — their profile hub. */
export function primaryPartyId(user: AuthUser | null | undefined): string | undefined {
  return verifiedPartyIds(user)[0];
}
