...
import { MAGAZINE_V2_ENABLED, PAGE_W, PAGE_H, MAX_PAGES_PER_ISSUE, MAX_SOURCE_BYTES, ALLOWED_SOURCE_MIME, sourceExtForMime } from '../lib/magazineV2/config.js';
...
router.post('/issues/:id/publish', async (req, res) => {
  ...
  const scope: 'full' | 'selected' = req.body?.scope === 'selected' ? 'selected' : 'full';
