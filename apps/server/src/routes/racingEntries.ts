...
router.post('/', async (req, res) => {
  const body = req.body as Partial<{ horse_id: string }>;
  ...
  const createdDoc = await db.collection('racingEntries').findById(id);
  if (!createdDoc) {
...
router.put('/:id', async (req, res) => {
  ...
  const found = await db.collection('racingEntries').findById(id);
  if (!found) {
  ...
  await db.collection('racingEntries').updateOne(id, updateData);
  const updated = await db.collection('racingEntries').findById(id);
  res.json(project(updated!));
});

// delete
router.delete('/:id', async (req, res) => {
  const deleted = await db.collection('racingEntries').deleteOne(String(req.params.id));
  if (!deleted) {
    res.status(404).json({ error: 'Not found' });
    return;
  }
